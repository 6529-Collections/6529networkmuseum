...
*Sixteen photographs selected in 2026 ask one shared question: how does a permission decide who may move, who must wait, and what remains outside the frame?*
...
The next movement is historical and infrastructural. A weathered wall carries graffiti after its barrier has weakened; an industrial compound carries painted words; water radiates through a fluted structure the artist identifies as a spillway; a train window frames two women against the memory of liberation. The final works place the checkpoint in the body and the interface: a refrigerator, a concealed torso, a small booklet or document held in a theatrical room, and an “Esc” key that promises an exit to a subject too small to use it.
...
The sequence supplies a curatorial order for reading the group. The final acquired quantity may be fewer than sixteen because availability, terms, rights, documentation, and program quantity remain open. The written scholarship is complete as a constructed editorial edition; exact-commit independent review, accessibility, image display, and sensitive-subject authorization remain pending. This current candidate is text-only: Work pages retain written captions, close looking, and source links but no direct Markdown image embeds; typed media references remain metadata-only with profile.media.visual: false, and constructed accessibility descriptions remain pending independent visual review. OUT-004, OUT-010, and OUT-011 images are not approved for delivery. OUT-004 remains unresolved because the depicted figures’ ages and consent/privacy scope are unverified; OUT-010 because identity, adult status, self-portrait status, and consent/publication scope remain unresolved; and OUT-011 because direct subject approval, document legibility, venue permission, sensitive biography, and publication scope remain open. The group’s current relationship is summarized below; the [Research and record](institutional-record.md) page keeps the supporting record.
...
HugoFaz appears twice in the selected group, in OUT-002 and OUT-011. Each work has a separate treatment; the artist has one canonical profile.

## Relationship and status

> Program: **Keys and Gates**, `6529NM-AP-01`  
...
The display title for OUT-015 follows the exact UTF-8 submission/media text: *মুক্তিযুদ্ধ - Fight for Freedom*. A compact [transcription and correction note](institutional-record.md#transcription-and-correction-note) records the source-layer history; the historical JSON remains unchanged.
