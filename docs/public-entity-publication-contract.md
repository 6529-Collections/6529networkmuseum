...
This is the compact adapter contract for the frontend and downstream public consumers. It is a source contract, not a governance or acquisition action.
...
| Proposed gift `6529NM-PG-2026-001`, signed Wave drop `002bfa4f-8416-48bf-b35e-38f354e9a9f0` | Curated Acquisition `6529NM-CA-2026-003`, current status `selected_by_museum_wave_acquisition_review_in_progress`, append-only prior `PARTICIPATORY` history, and Wave relation; no Collection relation |
| Retained Magnum Photos 75 proposal name and five object dossiers | Organization `6529NM-ORG-0002` and Project/Series `6529NM-PRJ-0006` at `/museum/network/projects/magnum-photos-75`, one evidence-bound originator/publisher relation, and five Project→Work context relations; token manifestations remain on independent Work records and do not change CA-003 or Collection state |
| Existing CloudFront program manifest, Casey evidence/observation records, and signed-Wave media | Typed `MEDIA_REFERENCE` entities and `ENTITY_HAS_MEDIA` relations for all 28 Works (7 Casey, 16 Keys and Gates, 5 Magnum); source, rights, fixity, accessibility, and affordances preserved |
