...
The same manifest retains an independently reviewed
[`generator-observations.json`](../evidence/casey-reas/generator-observations.json)
transcript. The finalizer reads the generator hashes, dependencies, control
maps, 923 EMPTY ROOMS count resolution, and Ex Nihilo implementation
discrepancy from that evidence file; the validator reads it independently and
binds its exact reviewed bytes. The transcript is evidence of the completed
technical review, not a claim that generator response bytes were retained.
Every object and condition report links the transcript by Git blob identity and
SHA-256.

The Ethereum receipt predates the donor declaration. It is recorded as custody
evidence at `2026-08-01T13:25:47Z`, not as proof that an offer already existed.
The object lifecycle begins `offered` at the formal donor declaration and
acceptance timestamp, `2026-08-01T22:55:00Z`; the co-temporal authorized,
acquired, and received-onchain administrative states then lead to accession on
2026-08-02. The certificate's acquisition event binds the exact accession-lot
bytes with an immutable Git blob URI and SHA-256.
...
The published descriptors are reproducible from frozen collection snapshots, a published method and configuration, and exact content hashes. They use the Museum's transparent NextGen-compatible distribution method. The historical `pending-descriptors.json` filename remains stable, but its current ledger records the completed exact-head independent review and per-output approvals. No OpenSea or other marketplace rarity metric is used, and no descriptor makes an aesthetic, quality, value, or ranking claim.
...
## Post-accession title, custody, and compliance diligence

Revision 2 of record `6529NM.2026.001.DILIGENCE-01` is constructed for independent exact-commit review without redating the gift or accession. If approved, it confirms that the public institutional title instrument is the executed title record for the accepted full-gift mode and that the unused restricted-annex stub does not create a hidden title gate. At Ethereum finalized block `25,667,060`, hash `0x01dc7575349d0893386928c218b64a11b8d71e42015b1995bafa7d65e05084e3`, all seven `ownerOf` results and `networkmuseum.6529.eth` resolved to the Museum address, while every token-specific `getApproved` result was the zero address. Every contract read used an EIP-1898 selector containing that exact block hash with `requireCanonical: true`, and all nineteen JSON-RPC responses are retained verbatim.

The same review records a point-in-time official OFAC Sanctions List Service exact-address screen. A known listed digital-currency address produced the expected positive result; the eight Museum-lot addresses produced no exact match. This is not identity verification, fuzzy-name screening, 50 Percent Rule analysis, transaction tracing, or a legal opinion. Official UI response bytes were not exportable and are therefore not represented as retained. The complete evidence boundary and residual-risk disposition are fixed in [`post-accession-diligence.json`](../records/accessions/6529NM.2026.001/post-accession-diligence.json), its [public note](../records/accessions/6529NM.2026.001/public/custody-title-and-compliance-diligence.md), and the [diligence manifest](../evidence/casey-reas-diligence/manifest.json).
