...
| Getty AAT/ULAN | `conceptual_mapping` | human-readable artist, medium, technique and work-type terms | reconcile Casey Reas to ULAN if available; assign reviewed AAT URIs and retain term labels/snapshots |
| IIIF Presentation 3.0 | `conceptual_mapping` | approved still URLs, live generator routes, credits, rights, dimensions and object ordering | preserve authorized image bytes; publish one collection Manifest and seven object Manifests |
| C2PA 2.4 | `conceptual_mapping` | source, creation context and fixity records for current documentation | create authorized derivatives; sign Museum assertions; retain manifests and trust-validation reports |
