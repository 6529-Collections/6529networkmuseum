...
| [`scripts/acquire_casey_custody_audit.py`](scripts/acquire_casey_custody_audit.py) | reproducible evidence acquisition | Exact-finalized-block ENS, `ownerOf`, and token-level `getApproved` observations using EIP-1898 block-hash selectors, exact JSON-RPC response retention, and stable canonical-block enforcement |
