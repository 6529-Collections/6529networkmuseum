...
Executable network retrieval is now routed through the merged PR #7 control
plane at `7193bfb9a0a6ead1871180b931aced755676b327`. The acquisition script uses
the repository's approved `scripts/safe_fetch.py` primitive for both bounded
JSON POST (JSON-RPC and Hasura) and GET requests; `scripts/check_fetch_guard.py`
is run in CI and the two control-plane modules are root-inventory-bound. The
current package remains an offline reconstruction of the already preserved v2
run; no raw observation bytes were refetched or regenerated by this migration.
